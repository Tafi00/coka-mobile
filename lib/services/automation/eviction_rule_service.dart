...
class EvictionRuleService {
  static const String _baseUrl = "${AutomationEndpoints.baseUrl}${AutomationEndpoints.automationBase}";
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['code'] == 0 && jsonData['content'] != null && jsonData['content'] is List) {
          ...
          print('⚠️ Eviction response structure unexpected: code=${jsonData['code']}, content=${jsonData['content']}');
        }
  ...
  static Future<ApiResponse<List<EvictionLog>>> getEvictionLogs(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final jsonData = json.decode(response.body);
        final List<EvictionLog> logs = (jsonData['data'] as List)
            .map((item) => EvictionLog.fromJson(item))
            .toList();
